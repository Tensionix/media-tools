...
# Kept from this project's own renderer: names its code imports that the
# reference does not provide, because they produce ANSI rather than render it.
ANSI_STATUS_COLORS = {
    "OK": 32,
    "INFO": 36,
    "WARN": 33,
...
def ansi_sgr(codes: str | int, text: str) -> str:
    return f"\x1b[{codes}m{text}\x1b[0m"


def ansi_status(label: str) -> str:
    clean = str(label or "").strip().upper()
    if not clean:
        return "[]"
    color = ANSI_STATUS_COLORS.get(clean)
    status = f"[{clean}]"
    return ansi_sgr(color, status) if color is not None else status
