...
- No NVIDIA GPU: install the FFmpeg 8.0.x Stable default. A machine without a
  card still builds the product for machines that have one, so an unmeasurable
  driver is not a reason to pick the narrowest branch.
