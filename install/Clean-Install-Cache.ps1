...
.DESCRIPTION
  ...
  Removed when present:
    - cached files/folders inside install\download\, except the preserve list
      below;
    - exact installer staging directories system_core\_pwsh_tmp,
      system_core\_fzf_tmp, system_core\_ffmpeg_btbn_tmp and
      system_core\_deno_tmp;
...
$stagingDirs = @(
    ...
    (Join-ProjectPath 'system_core\_ffmpeg_btbn_tmp'),
    (Join-ProjectPath 'system_core\_deno_tmp')
)
