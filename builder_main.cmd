...
title Audion Media Tools - Builder
...
rem config\version.json is the only place the version is decided; the YAML
rem configs are refreshed from it here so a second copy cannot go stale.
call :SYNC_VERSION

:SYNC_VERSION
set "SYNC_PS="
if exist "%BASE_DIR%\system_core\powershell\pwsh.exe" set "SYNC_PS=%BASE_DIR%\system_core\powershell\pwsh.exe"
if not defined SYNC_PS where pwsh.exe >nul 2>nul && set "SYNC_PS=pwsh.exe"
if not defined SYNC_PS where powershell.exe >nul 2>nul && set "SYNC_PS=powershell.exe"
if not defined SYNC_PS exit /b 0
if not exist "%BASE_DIR%\install\Sync-ProjectVersion.ps1" exit /b 0
"%SYNC_PS%" -NoLogo -NoProfile -ExecutionPolicy Bypass -File "%BASE_DIR%\install\Sync-ProjectVersion.ps1" -ProjectRoot "%BASE_DIR%" >nul 2>nul
exit /b 0

:MAIN
call :CLEAR_SCREEN
echo ======================================================================
echo   AUDION MEDIA TOOLS - BUILDER
echo ======================================================================
echo Root: %BASE_DIR%
echo.
...
>>"%MENU_FILE%" echo.
>>"%MENU_FILE%" echo [10] 7-ZIP                                 ^| install_7zip      ^| install portable 7-Zip
>>"%MENU_FILE%" echo [11] FFMPEG BTBN AUTO-FALLBACK             ^| install_ffmpeg_btbn ^| Driver-aware Stable branch; Gyan provider fallback
>>"%MENU_FILE%" echo [12] FFMPEG GYAN                           ^| install_ffmpeg_gyan ^| install portable FFmpeg from Gyan
>>"%MENU_FILE%" echo [13] YT-DLP                                ^| install_ytdlp     ^| install portable yt-dlp plus Deno
>>"%MENU_FILE%" echo [14] MPV                                   ^| install_mpv       ^| install portable mpv with its GPL sources
>>"%MENU_FILE%" echo.
...
if /I "%RAW%"=="install_ytdlp" goto INSTALL_YTDLP
if /I "%RAW%"=="install_mpv" goto INSTALL_MPV
...
echo.
echo [10] 7-ZIP
echo [11] FFMPEG BTBN AUTO-FALLBACK
echo [12] FFMPEG GYAN
echo [13] YT-DLP
echo [14] MPV
echo.
...
if /I "%RAW%"=="install_offline" goto INSTALL_OFFLINE
if /I "%RAW%"=="10" goto INSTALL_7ZIP
if /I "%RAW%"=="install_7zip" goto INSTALL_7ZIP
if /I "%RAW%"=="11" goto INSTALL_FFMPEG_BTBN
if /I "%RAW%"=="install_ffmpeg_btbn" goto INSTALL_FFMPEG_BTBN
if /I "%RAW%"=="12" goto INSTALL_FFMPEG_GYAN
if /I "%RAW%"=="install_ffmpeg_gyan" goto INSTALL_FFMPEG_GYAN
if /I "%RAW%"=="install_ffmpeg" goto INSTALL_FFMPEG_BTBN
if /I "%RAW%"=="13" goto INSTALL_YTDLP
if /I "%RAW%"=="install_ytdlp" goto INSTALL_YTDLP
if /I "%RAW%"=="14" goto INSTALL_MPV
if /I "%RAW%"=="install_mpv" goto INSTALL_MPV
...
:INSTALL_YTDLP
call "%BASE_DIR%\install\Install-Portable-yt-dlp.cmd"
goto MAIN

:INSTALL_MPV
call "%BASE_DIR%\install\Install-Portable-mpv.cmd"
goto MAIN
...
:CLEAR_SCREEN
cls
if not defined BUILDER_ESC call :SET_ESC
if defined BUILDER_ESC <nul set /p "=%BUILDER_ESC%[3J"
cls
goto :eof

:SET_ESC
for /F "tokens=1 delims=#" %%E in ('"prompt #$E# & echo on & for %%F in (1) do rem"') do set "BUILDER_ESC=%%E"
goto :eof
